"""
Baseline for TDC DrugRes (GDSC1) - RANDOM SPLIT
...
class Config:
    # Experiment tagging
    run_tag: str ="random_split"  # <-- Άλλαξε το tag
...
def main() -> None:
    ...
    # ----------------------------
    # RANDOM SPLIT (Warm Split)
    # ----------------------------
    idx = np.arange(len(df))
    
    # Η ΑΛΛΑΓΗ ΕΙΝΑΙ ΕΔΩ: Απλό τυχαίο split
    idx_train, idx_val = train_test_split(
        idx, test_size=cfg.val_size, random_state=cfg.seed, shuffle=True
    )

    # Απλά τυπώνουμε το overlap για να δεις τη διαφορά (δεν υπάρχει assert που να το σταματάει)
...
# -------------------------------------------------------
    ...
    metrics = {
        "run_id": run_id,
        "split_type": "random_split",  # <-- Ενημερώθηκε το split type στα logs
