...
class Config:
    # Experiment tagging
    run_tag: str = "gine_jk_concat_readout_projection"  # <-- Updated to reflect what we are actually doing!
...
class DrugGNNEncoder(torch.nn.Module):
    ...
    def forward(self, drug_graph):
        ...
        z1 = gap(x1, batch)   # [B, hidden_dim]
        z2 = gap(x2, batch)   # [B, hidden_dim]
        z3 = gap(x3, batch)   # [B, hidden_dim]

        z = torch.cat([z1, z2, z3], dim=1)
        z_drug = self.readout_proj(z)
        return z_drug
