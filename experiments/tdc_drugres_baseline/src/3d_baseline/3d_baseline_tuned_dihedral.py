...
from rdkit.Chem import AllChem, DataStructs, rdMolTransforms
...
@dataclass
class Config:
    run_tag: str = "geognn_dihedral_v2"
    ...
    cache_dir: str = ""
    graph_cache_file: str = "gdsc1_geognn_3d_v2_dihedral.pt"
    results_dir: str = ""
...
# ----------------------------
# Dihedral / Torsion Angle Calculation
# ----------------------------
def get_canonical_torsion(mol, conf, x_atom, y_atom):
    """
    GeoMol §2.4: ένα canonical torsion angle ανά μη-τερματικό δεσμό X-Y,
    permutation invariant ως προς τους γείτονες, με c_ij = 1.
    Επιστρέφει (sin(alpha), cos(alpha)), ή (0,0) αν ο δεσμός είναι τερματικός.
    """
    x = mol.GetAtomWithIdx(x_atom)
    y = mol.GetAtomWithIdx(y_atom)

    t_neighbors = [a.GetIdx() for a in x.GetNeighbors() if a.GetIdx() != y_atom]
    z_neighbors = [a.GetIdx() for a in y.GetNeighbors() if a.GetIdx() != x_atom]

    if not t_neighbors or not z_neighbors:
        return 0.0, 0.0  # τερματικός δεσμός -> δεν ορίζεται torsion

    s_cos, s_sin = 0.0, 0.0
    for ti in t_neighbors:
        for zj in z_neighbors:
            try:
                delta = rdMolTransforms.GetDihedralRad(conf, ti, x_atom, y_atom, zj)
            except Exception:
                continue  # π.χ. σχεδόν συγγραμμικά άτομα
            s_cos += np.cos(delta)
            s_sin += np.sin(delta)

    norm = np.sqrt(s_cos**2 + s_sin**2)
    if norm < 1e-6:
        return 0.0, 0.0  # symmetric cancellation (σπάνια περίπτωση, βλ. paper)

    return s_sin / norm, s_cos / norm
...
def smiles_to_graph_3d(smiles: str):
    ...
    for bond in new_mol.GetBonds():
        ...
        sin_a, cos_a = get_canonical_torsion(new_mol, conf, i, j)
...
class DrugResponsePredictor(nn.Module):
    def __init__(self, cell_in: int, cfg: Config):
        ...
        self.drug_gnn = DrugGeoEncoder(
            atom_feat_dim=9, bond_feat_dim=8,
