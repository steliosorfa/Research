...
@dataclass
class Config:
    run_tag: str = "final_3d_geognn_MBCA"  # used in results dir
...
# ----------------------------
# Cell Tokenizer (Chunked)
# ----------------------------    

class ChunkedCellTokenizer(nn.Module):
    def __init__(self, n_genes=1000, n_tokens=10, d_model=128):
        super().__init__()
        assert n_genes % n_tokens == 0
        self.chunk_size = n_genes // n_tokens          # 100
        self.n_tokens   = n_tokens
        self.proj       = nn.Linear(self.chunk_size, d_model)
        self.pos        = nn.Parameter(torch.zeros(1, n_tokens, d_model))  # learned positional

    def forward(self, x_cell):                          # x_cell: [B, 1000]
        x = x_cell.view(-1, self.n_tokens, self.chunk_size)   # [B, 10, 100]
        x = self.proj(x) + self.pos                            # [B, 10, d_model]
        return x, None                                          # no mask, all real tokens
...
class MBCABlock(nn.Module):
    ...
    def forward(self, drug_tok, drug_mask, cell_tok, cell_mask):
        # PyTorch expects True = position to IGNORE, so invert "valid" masks
        drug_kp = ~drug_mask if drug_mask is not None else None
        cell_kp = ~cell_mask if cell_mask is not None else None

        # drug attends to cell
        d_upd, _   = self.d2c(drug_tok, cell_tok, cell_tok, key_padding_mask=cell_kp)
        drug_tok   = self.ln_d1(drug_tok + d_upd)
        drug_tok   = self.ln_d2(drug_tok + self.ffn_d(drug_tok))

        # cell attends to drug (use ORIGINAL drug_tok, not the updated one — cleaner gradients)
        c_upd, _   = self.c2d(cell_tok, drug_tok, drug_tok, key_padding_mask=drug_kp)
...
class DrugResponsePredictor(nn.Module):
    def __init__(self, cell_in: int, cfg: Config):
        ...
        self.cell_tokenizer = ChunkedCellTokenizer(
            n_genes=cell_in, n_tokens=cfg.cell_n_tokens, d_model=d_model)

        # Two AMDRP-style branches
        self.mbca_atom = MBCABlock(d_model, cfg.mbca_heads, cfg.dropout)
        self.mbca_bond = MBCABlock(d_model, cfg.mbca_heads, cfg.dropout)

        # After pooling, we have 4 vectors of dim d_model:
        #   h_atom, h_cell_from_atom, h_bond, h_cell_from_bond
        fused_dim = d_model * 4
    ...
    def forward(self, atom_graph, bond_graph, x_cell):
        ...
        # Branch 2: bond ↔ cell (fresh copy of cell tokens — don't reuse the atom branch's output)
        ...
        fused = torch.cat([h_atom, h_cell_a, h_bond, h_cell_b], dim=1)
