...
@dataclass
class Config:
    run_tag: str = "hybrid_ecfp_gine_v3"
...
def build_or_load_ecfp_cache(cfg: Config, smiles_list: List[str]) -> np.ndarray:
    ...
    if os.path.exists(cache_path):
        ...
        if meta.get("n") == len(smiles_list) and meta.get("bits") == cfg.ecfp_bits:
...
class HybridDrugResDataset(Dataset):
    def __init__(self, drug_graphs: List[Data], ecfp_array: np.ndarray, cell_expr: np.ndarray, y: np.ndarray):
        self.drug_graphs = drug_graphs
        self.ecfp_array = ecfp_array.astype(np.float32)
        self.cell_expr = cell_expr.astype(np.float32)
    ...
    def __getitem__(self, idx: int):
        ...
        x_ecfp = torch.from_numpy(self.ecfp_array[idx])
        x_cell = torch.from_numpy(self.cell_expr[idx])
        y_val = torch.tensor(self.y[idx], dtype=torch.float32).view(1)
        return graph, x_ecfp, x_cell, y_val
...
class HybridPredictor(nn.Module):
    def __init__(self, node_feat_dim: int, ecfp_dim: int, cell_in: int, cfg: Config):
        super().__init__()
        ...
        self.ecfp_mlp = MLP(
            input_dim=ecfp_dim,
            hidden_dim=cfg.ecfp_hidden,
            output_dim=cfg.ecfp_z_dim,
            dropout=cfg.dropout
        )
        ...
        fusion_dim = cfg.z_dim + cfg.ecfp_z_dim + cfg.z_dim  # 128 + 64 + 128 = 320
    ...
    def forward(self, drug_graph, x_ecfp: torch.Tensor, x_cell: torch.Tensor) -> torch.Tensor:
        z_graph = self.drug_gnn(drug_graph)
        z_ecfp = self.ecfp_mlp(x_ecfp)
        z_cell = self.cell_mlp(x_cell)

        z = torch.cat([z_graph, z_ecfp, z_cell], dim=1)
        return self.head(z)
...
def train_one_epoch(model, loader, opt, loss_fn, device) -> float:
    ...
    total = 0.0
    n = 0
    for drug_graph, x_ecfp, x_cell, y in loader:
        # Move the entire graph batch to the GPU
        drug_graph = drug_graph.to(device)
        x_ecfp = x_ecfp.to(device)
        x_cell = x_cell.to(device)
        y = y.to(device)

        opt.zero_grad(set_to_none=True)
        y_hat = model(drug_graph, x_ecfp, x_cell)
...
def predict(model, loader, device) -> Tuple[np.ndarray, np.ndarray]:
    model.eval()
    preds, targets = [], []
    for drug_graph, x_ecfp, x_cell, y in loader: # Unpack 4 items here!
        drug_graph = drug_graph.to(device)
        x_ecfp = x_ecfp.to(device) # Send ECFP to device
        x_cell = x_cell.to(device)
        
        # Pass all 3 inputs: graph, ecfp, and cell
        y_hat = model(drug_graph, x_ecfp, x_cell).detach().cpu().view(-1)
...
def main() -> None:
    ...
    # ----------------------------
    # Multimodal Drug Processing (Graph + ECFP)
    # ----------------------------
    smiles = df["Drug"].astype(str).tolist()
    y = df["Y"].astype(float).to_numpy()
    
    print("Loading multimodal drug caches (Graph & ECFP)...")
    # Load BOTH caches!
    ecfp_array = build_or_load_ecfp_cache(cfg, smiles)
    drug_graphs = build_or_load_graph_cache(cfg, smiles)

    # (Note: We removed the manual cell_expr standardization here 
    # because the StandardScaler inside your PCA block already handles it!)

    # Datasets using the new Hybrid format
    train_ds = HybridDrugResDataset(
        drug_graphs=[drug_graphs[i] for i in idx_train], 
        ecfp_array=ecfp_array[idx_train], 
        cell_expr=cell_expr[idx_train], 
        y=y[idx_train]
    )
    
    val_ds = HybridDrugResDataset(
        drug_graphs=[drug_graphs[i] for i in idx_val], 
        ecfp_array=ecfp_array[idx_val], 
        cell_expr=cell_expr[idx_val], 
        y=y[idx_val]
    )

    # PyG DataLoaders (PyG's DataLoader perfectly handles mixing PyG Graphs with standard tensors)
    ...
    model = HybridPredictor(
        node_feat_dim=8, 
        ecfp_dim=cfg.ecfp_bits, 
        cell_in=cfg.top_k_genes,   # <-- Make sure this is PCA_COMPONENTS and not TOP_K!
        cfg=cfg
    ).to(device)
