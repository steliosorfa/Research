...
def eval_epoch(model, loader, loss_fn, device, y_min=None, y_max=None):
    ...
    rmse_val = rmse(pred, target)   # already in ln(IC50) units — no denorm needed
    p        = pearsonr(pred, target)

    return total / max(n, 1), rmse_val, p
...
def save_learning_plots(history, run_dir):
    ...
    val_rmse   = [h["val_rmse_lnIC50"] for h in history]  # ← μόνο αυτό
...
def main() -> None:
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        print(f"Epoch {epoch:02d}/{cfg.epochs} "
            ...
            f"| val_RMSE_lnIC50={val_rmse:.4f} "    # ← απευθείας σε ln(IC50)
        ...
        history.append({
            ...
            "train_loss": tr_loss,
            "val_loss": val_loss,
            "val_rmse_lnIC50": val_rmse,    # ← ένα μόνο rmse τώρα
            "val_pearson": val_p,
...
# -------------------------------------------------------
    ...
    metrics = {
        ...
        "best_val_rmse_lnIC50": float(best_val_rmse), 
