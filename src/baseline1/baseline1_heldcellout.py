...
Cell Line gene expression vector ->Variance-based gene selection on TRAIN only -> MLP -> z_cell
Leave some cells out for testing, and train on the rest of the cells.
...
def main() -> None:    
    ...
    #----------------------------
    #Leave-cell-out split(Blind cell lines)
    #----------------------------
    idx = np.arange(len(df))
    
    # Group by cell line this time!
    ...
    train_cells = set(groups[idx_train])
    val_cells = set(groups[idx_val])
    overlap = train_cells.intersection(val_cells)
    
    print(f"[Split] Train samples: {len(idx_train)} | Val samples: {len(idx_val)}")
    print(f"[Split] Unique cell lines train: {len(train_cells)} | val: {len(val_cells)} | overlap: {len(overlap)}")
    assert len(overlap) == 0 , "Leave-Cell-Out violated: same Cell Line appears in train and val!"
